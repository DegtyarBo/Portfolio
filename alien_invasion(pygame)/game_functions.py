...
def check_play_button(ai_settings, screen, stats, play_button, ship, aliens, bullets, mouse_x, mouse_y, sb):
	...
	if button_clicked and not stats.game_active:
		...
		#указатель мыши скрываеться во время игры
		pygame.mouse.set_visible(False)
		#чистит статистику игры
		stats.reset_stats()
...
def check_keydown_events(event, ai_settings, screen, ship, bullets):
	...
	#Закрытие игры при нажатие кнопки q
...
def update_screen(ai_settings, screen, ship, aliens, bullets, stats, play_button, sb):
	...
	#Все пули выходят позади изображений корабля и пришельцев
	...
	# Отображение последнего зарисованного экрана
	pygame.display.flip()
...
def update_bullets(aliens, bullets, ai_settings, screen, ship, stats, sb):
	...
	bullets.update()
	#Удаление пуль ушедших за экран
	for bullet in bullets.copy():
		if bullet.rect.bottom <= 0:
			bullets.remove(bullet)
	#обработка столкновений пуль с пришельцами
	check_bullet_alien_collisions(aliens, bullets, ai_settings, screen, ship, stats, sb)
	
	
		
def check_bullet_alien_collisions(aliens, bullets, ai_settings, screen, ship, stats, sb):
	'''обработка столкновений пуль с пришельцами'''
	...
	if len(aliens) == 0:
		#если весь флот унечтожен, переход на следующий уровень
		bullets.empty()
...
def change_fleet_direction(ai_settings, aliens):
	'''Опускает и меняет направление флота'''
	for alien in aliens.sprites():
		alien.rect.y += ai_settings.fleet_drop_speed
	ai_settings.fleet_direction *= -1
	
def ship_hit(ai_settings, stats, screen, ship, aliens, bullets, sb):
	'''Обработка столкновения корабля с пришелцем'''
	if stats.ship_left > 0:
		#уменьшение жизней ships_left
		stats.ship_left -=1
		#обновление игровой тнформации
		sb.prep_ships()
		#очистка списков пришелцев и пуль
		aliens.empty()
		bullets.empty()
		#создание нового флота и размещение корабля в центре
		create_fleet(ai_settings, screen, ship, aliens)
